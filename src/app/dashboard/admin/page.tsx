...
export default function AdminDashboard() {
  ...
  const { data: all } = useCollection(db ? collection(db, 'drivers') : null);

  const stats = {
    total: all?.length || 0,
    approved: all?.filter((r: any) => r.verificationStatus === 'approved').length || 0,
    pending: all?.filter((r: any) => r.verificationStatus === 'pending').length || 0,
  };
    ...
    <div className="min-h-screen bg-slate-50 flex font-body">
      ...
      <aside className="w-80 bg-[#0F172A] text-white flex flex-col h-screen sticky top-0 shadow-3xl">
        <div className="p-10">
          ...
          <p className="text-[10px] font-bold text-white/40 uppercase tracking-[0.4em] mt-1">Admin Panel</p>
        </div>
        <nav className="flex-1 px-6 space-y-2">
          {[
            ...
            { id: 'monitoring', icon: MapIcon, label: 'Monitoring' }
          ].map((item) => (
            <Button 
              key={item.id}
              variant="ghost" 
      ...
      <main className="flex-1 p-12 overflow-y-auto no-scrollbar">
        <header className="flex justify-between items-center mb-12">
          <div className="space-y-1">
            <h2 className="text-4xl font-black italic uppercase tracking-tighter text-[#0F172A]">System Overview</h2>
            <p className="text-slate-400 font-bold italic">Real-time status of Mutambuke network</p>
          </div>
          <div className="flex items-center gap-6">
            <div className="relative w-80">
              <Input className="pill-input bg-white shadow-sm" placeholder="Search ID or Plate..." />
        ...
        <div className="grid grid-cols-4 gap-8 mb-16">
          <StatCard icon={Bike} label="Total Riders" value={stats.total} color="text-[#0F172A]" bg="bg-slate-100" />
          <StatCard icon={CheckCircle2} label="Approved" value={stats.approved} color="text-secondary" bg="bg-secondary/10" />
          <StatCard icon={Clock} label="Pending" value={stats.pending} color="text-accent" bg="bg-accent/10" />
          <StatCard icon={TrendingUp} label="Active Rides" value={124} color="text-blue-600" bg="bg-blue-50" />
        </div>

        {/* CONTENT SWITCH */}
        {activeNav === 'riders' ? (
          <section className="space-y-6">
            <h3 className="text-2xl font-black italic uppercase text-[#0F172A]">{translations.en.verifyRiders}</h3>
            <div className="grid gap-6">
              {pending?.map((r: any) => (
                <Card key={r.driverId} className="rounded-[2.5rem] border-none shadow-xl bg-white p-8 flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-400">{r.vehicleType.toUpperCase()} • {r.licenseCategory}</p>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <Button onClick={() => updateVerification(r.driverId, 'rejected')} variant="outline" className="h-16 px-10 rounded-2xl font-black text-red-500 border-red-100">Decline</Button>
                    <Button onClick={() => updateVerification(r.driverId, 'approved')} className="h-16 px-10 rounded-2xl bg-secondary text-white font-black italic">Approve Rider</Button>
                  </div>
                </Card>
              ))}
              {!pending?.length && <div className="py-20 text-center text-slate-300 font-bold text-xl italic">No pending riders</div>}
            </div>
          </section>
        ) : (
          <div className="bg-white rounded-[3rem] p-20 text-center border-4 border-dashed border-slate-100">
            ...
            <h3 className="text-2xl font-black text-slate-300 italic uppercase">Dashboard Coming Soon</h3>
          </div>
        )}
      </main>
    </div>
